...
namespace SnTerminal
{
    ...
    public class CommandShell
    {
        ...
        {
            SplitCommand(input, out var commandLine, out var args);
            
            List<string> suggestions = new List<string>();

            // Check all commands starting with the input
            foreach (var command in commands)
            {
                if (command.Key.StartsWith(commandLine.ToUpper()))
                {
                    // Check if the command has the required number of arguments
                    if (command.Value.min_arg_count <= args.Length &&
                        (command.Value.max_arg_count == -1 || command.Value.max_arg_count >= args.Length))
                    {
                        string cmd = command.Key.ToLower();
                        
                        // add the arguments to the command
                        if (command.Value.max_arg_count > 0)
                        {
                            for (int i = 97; i < 97 + command.Value.max_arg_count; i++)
                            {
                                // a then b then c ...
                                cmd = $"{cmd} <arg: {(char)i}>";
                            }
        ...
        public void SplitCommand(string input, out string command, out string[] args)
        {

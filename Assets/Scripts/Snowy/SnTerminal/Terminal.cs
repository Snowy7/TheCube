...
namespace SnTerminal
{
    ...
    public class Terminal : MonoSingleton<Terminal>
    {
        ...
        void OnGUI() {
            ...
            suggestion_window = GUILayout.Window(89, suggestion_window, DrawSuggestions, "", window_style);
        }
        ...
        void SetupWindow() {
            ...
            suggestion_window = new Rect(0, real_window_size + current_open_t,
                Screen.width, suggestions.Length * ConsoleFont.fontSize);
        ...
        void DrawSuggestions(int Window2D)
        {
            if (command_text.Length == 0)
                return;
            
            if (pre_command_text != command_text)
            {
                pre_command_text = command_text;
                suggestions = Shell.GetSuggestions(command_text);
            }

            // A box under the input field
            GUILayout.BeginHorizontal(new []
            {
                GUILayout.Width(Screen.width),
                GUILayout.Height(ConsoleFont.fontSize * suggestions.Length)
            });
            
            GUILayout.Space(ConsoleFont.fontSize);
            GUILayout.BeginVertical();
            foreach (var suggestion in suggestions)
            {
                // Draw under the window
                GUILayout.Label(suggestion, new GUIStyle(label_style)
                {
                    normal = { textColor = InputColor },
                    // WIDTH
                    wordWrap = false,
                });
            }

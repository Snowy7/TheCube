...
using SnowyEditor.Window.ShapeWizards;
using SnowyEditor.Utils;
using UnityEditor;
using UnityEngine;
using Window;

namespace SnowyEditor.MenuItems
{
    internal static class BaseMenuItems
    {
        public const string MAIN_MENU_NAME = "Tools/" + LibConstants.LIB_NAME + "/";
        
        [MenuItem(MAIN_MENU_NAME + "Scene Loader")]
        private static void OpenSceneLoader()
        {
            SceneLoader window = EditorWindow.GetWindow<SceneLoader>("Scene Loader");
            window.minSize = new Vector2(300, 300);
        }

        [MenuItem(MAIN_MENU_NAME + "/Assets/Bootstrap")]
        private static void CreateBootstrapWindow()
        {
            BootstrapWindow window = EditorWindow.GetWindow<BootstrapWindow>("Bootstrap Window");
